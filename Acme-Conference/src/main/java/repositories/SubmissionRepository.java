...
public interface SubmissionRepository extends JpaRepository<Submission, Integer> {
	...
	//@Query("select s from Submission s where ?1 in s.reviewers")
	Collection<Submission> findByReviewer2(Reviewer reviewer);

	@Query("select s from Submission s where s.status = 'UNDER-REVIEW' and ?1 member of s.reviewers")
	Collection<Submission> findByReviewerUnderReview(Reviewer reviewer);
